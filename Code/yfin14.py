...
if st.sidebar.button('Xem thông tin dữ liệu'):
    ...
    try:
        ticker = yf.Ticker(selected_company_code)
        df = ticker.history(start=selected_date_start, end=selected_date_end).iloc[:, :-2]
...
model_Apple = pickle.load(open('Model_data/Model_Apple.pkl', 'rb'))
model_Amazon = pickle.load(open('Model_data/Model_Amazon.pkl', 'rb'))
model_Meta = pickle.load(open('Model_data/Model_Meta.pkl', 'rb'))
model_Google = pickle.load(open('Model_data/Model_Google.pkl', 'rb'))
model_Microsoft = pickle.load(open('Model_data/Model_Microsoft.pkl', 'rb'))
...
if st.sidebar.button("Dự báo"):
    # Kiểm tra nếu các giá trị đầu vào hợp lệ
    if open_price > 0 and high_price > 0 and low_price >= 0 and close_price > 0:
        ...
        # Chuyển giá trị predicted_price thành float (nếu nó không phải là float)
        predicted_price = float(predicted_price)

        # Lưu giá trị dự đoán vào session_state
        st.session_state.predicted_prices.append(predicted_price)

        # Tạo chuỗi định dạng giá trị
        formatted_price = "${:,.2f}".format(predicted_price)
        st.markdown('<p class="red-text">Dự đoán giá cổ phiếu: ' + formatted_price + '</p>', unsafe_allow_html=True)
        ...
        if st.session_state.predicted_prices:
            ...
            for i, price in enumerate(st.session_state.predicted_prices):
                # Kiểm tra kiểu dữ liệu trước khi định dạng
                if isinstance(price, (int, float)):  # Kiểm tra nếu price là kiểu số
                    formatted_price = "${:,.2f}".format(price)
                    st.markdown('<p class="edit-text_yellow">Dự đoán ' + str(i + 1) + ': ' + formatted_price + '</p>',
                                unsafe_allow_html=True)
                else:
                    st.error(f"Dữ liệu không hợp lệ tại dự đoán {i + 1}")

        # Kiểm tra nếu danh sách dự đoán không trống và chứa các giá trị hợp lệ
        if len(st.session_state.predicted_prices) > 0 and all(
                isinstance(p, (int, float)) for p in st.session_state.predicted_prices):
            # Vẽ biểu đồ đường
            plt.figure(figsize=(10, 6), facecolor='black')
            plt.plot(range(1, len(st.session_state.predicted_prices) + 1), st.session_state.predicted_prices,
                     color='lime')
            ...
            st.error("Không có giá trị dự đoán hợp lệ để vẽ biểu đồ.")
    else:
        st.error("Vui lòng nhập đầy đủ thông tin hợp lệ để dự báo.")
